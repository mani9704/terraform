...
module "front_door" {
  ...
  origins = [
    for origin in lookup(each.value, "origins", []) : {
      ...
      # Priority: app_service_key > vm_key > target_resource_id > host_name
      # If app_service_key is provided, use App Service resource ID (recommended for App Services)
      target_resource_id = origin.app_service_key != null && length(module.app_service) > 0 && contains(keys(module.app_service), origin.app_service_key) ? (
        module.app_service[origin.app_service_key].app_service_id
      ) : (
        # If vm_key is provided, use VM resource ID (for Private Link or direct reference)
        origin.vm_key != null && lookup(origin, "use_vm_resource_id", false) && length(module.virtual_machine) > 0 && contains(keys(module.virtual_machine), origin.vm_key) ? (
          module.virtual_machine[origin.vm_key].vm_id
        ) : (
          # Otherwise use direct target_resource_id if provided
          lookup(origin, "target_resource_id", null)
        )
      )
      
      # If target_resource_id is not set, use host_name (for VMs with public IP or external endpoints)
      host_name = lookup(origin, "target_resource_id", null) == null && lookup(origin, "app_service_key", null) == null ? (
...
module "app_service" {
  ...
  # Site Config - Auto-inject Application Insights if app_insights_key is provided
  site_config = merge(
    lookup(each.value, "site_config", {}),
    lookup(each.value, "app_insights_key", null) != null && length(module.application_insights) > 0 ? (
      contains(keys(module.application_insights), each.value.app_insights_key) ? {
        application_insights_connection_string = module.application_insights[each.value.app_insights_key].application_insights_connection_string
        application_insights_key               = module.application_insights[each.value.app_insights_key].application_insights_instrumentation_key
      } : {}
    ) : {}
  )
