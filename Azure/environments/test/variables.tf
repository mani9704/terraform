...
variable "front_doors" {
  ...
  type = map(object({
    ...
    origins = optional(list(object({
      ...
      app_service_key = optional(string, null) # Reference to App Service key (uses App Service default hostname)
      vm_key          = optional(string, null) # Reference to VM key (uses VM public IP)
...
variable "app_services" {
  ...
  type = map(object({
    ...
    # will be automatically added to app_settings
...
variable "storage_accounts" {
  description = "Map of Storage Accounts to create. Key is used as reference identifier."
  type = map(object({
